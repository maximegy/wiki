...
$outputFolder = 'C:\Users\boola\Documents\test'
...
Start-Job -Name FailJob -ScriptBlock {New-Item -Path 'C:\' -Name 'test' -ItemType Directory -ErrorAction Stop} 
Start-Job -Name FailCompletedJob -ScriptBlock {New-Item -Path 'C:\' -Name 'test' -ItemType Directory} 
...
"@
    ...
    $ourJobs = Get-Job 
    #If jobs exists, keep the loop running by setting $jobs to $true, else set it to $fals
